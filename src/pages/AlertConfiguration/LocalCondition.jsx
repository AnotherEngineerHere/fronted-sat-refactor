...
const LocalCondition = () => {
  ...
  const {
    ...
    error: requestError,
    requestLoading,
  } = useGetLocalConditionsQuery(userInfo);
  const {
    data: preconData,
    error: preconError,
    preconLoading,
  ...
  const [createLocalConditionReq, { isLoading: createLoading }] =
    useCreateLocalConditionMutation();
  const [updateLocalConditionReq, { isLoading: updateLoading }] =
    useUpdateLocalConditionMutation();
  const [deleteLocalConditionReq, { isLoading: deleteLoading }] =
    useDeleteLocalConditionMutation();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (values.typeForm === "create") {
        ...
        await createLocalConditionReq({ userInfo, localConditon });
      } else {
        const localConditon = {
  ...
  const setForkiFields = (values, from) => {
    if (from === "create") {
      formik.setFieldValue("loconColumnname", "");
      formik.setFieldValue("loconKeycolumn", "");
      formik.setFieldValue("loconOperator", "");
      formik.setFieldValue("loconQuerystring", "");
      formik.setFieldValue("loconTablename", "");
      formik.setFieldValue("loconValuetype", "");
      formik.setFieldValue("preconId", "");
      formik.setFieldValue("thresId", "");
    } else if (from === "edit") {
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <Row gutter={10} style={{ marginTop: 20 }}>
          <Col xs={12} md={12}>
            ...
            <Select
              ...
              defaultValue=">"
              value={formik.values.loconOperator}
        ...
        <Row gutter={10} style={{ marginTop: 20 }}>
          ...
          <Col xs={12} md={12}>
            ...
            <Select
              className="ant-select-selection"
              defaultValue="Cadena"
              value={formik.values.loconValuetype}
        ...
        <Row gutter={10} style={{ marginTop: 20 }}>
          <Col xs={12} md={12}>
            ...
            >
              {preconData &&
                preconData.map((precon) => (
                  <Option key={precon.preconId}>
                    id:{precon.preconId} ope: {precon.preconLogicaloperand}
                  </Option>
                ))}
            </Select>
          ...
          <Col xs={12} md={12}>
            <LabelInput labelText={"Threshold"} nameInput={"thresId"} />
            <Select
              className="ant-select-selection"
              value={formik.values.thresId}
    ...
    <SectionTitle title={"Condiciones Locales"}>
      ...
      <Modal
        title={
          typeModal === "create"
            ? `Crear Condiciones Locales`
            : `Editar Condiciones Locales # ${itemSelected}`
        }
        visible={showModal}
        onOk={() => {}}
