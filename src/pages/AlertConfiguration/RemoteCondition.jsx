...
const RemoteCondition = () => {
  ...
  const {
    ...
    error: requestError,
    requestLoading,
  } = useGetRemoteConditionQuery(userInfo);
  const {
    data: preconData,
    error: preconError,
    preconLoading,
  ...
  const [createRemoteConditionReq, { isLoading: createLoading }] =
    useCreateRemoteConditionMutation();
  const [updateRemoteConditionReq, { isLoading: updateLoading }] =
    useUpdateRemoteConditionMutation();
  const [deleteRmoteConditionReq, { isLoading: deleteLoading }] =
    useDeleteRemoteConditionMutation();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (values.typeForm === "create") {
        ...
        await createRemoteConditionReq({ userInfo, remoteCondition });
      } else {
        const remoteCondition = {
  ...
  const deleteRemotecondition = async (remconId) => {
    await deleteRmoteConditionReq({ remconId, userInfo });
  };
  ...
  const setForkiFields = (values, from) => {
    if (from === "create") {
      formik.setFieldValue("remconDelegatename", "");
      formik.setFieldValue("remconMethodname", "");
      formik.setFieldValue("remconParametername", "");
      formik.setFieldValue("remconReturnvaluetype", "");
      formik.setFieldValue("preconId", "");
      formik.setFieldValue("thresholdId", "");
    } else if (from === "edit") {
      ...
      formik.setFieldValue(
        "remconReturnvaluetype",
        values.remconReturnvaluetype
      );
    ...
    <div>
      <form onSubmit={formik.handleSubmit}>
        ...
        <Row gutter={10} style={{ marginTop: 20 }}>
          <Col xs={12} md={12}>
            <LabelInput
              labelText={"Nombre método"}
              nameInput={"remconMethodname"}
            />
            <Input
              placeholder=""
          ...
          <Col xs={12} md={12}>
            ...
            <Select
              ...
              value={formik.values.remconReturnvaluetype}
        ...
        <Row gutter={10} style={{ marginTop: 20 }}>
          <Col xs={12} md={12}>
            <LabelInput labelText={"Precondición"} nameInput={"preconId"} />
            <Select
              className="ant-select-selection"
              value={formik.values.preconId}
              onChange={preconHandleChange}
            >
              {preconData &&
                preconData.map((valType) => (
                  <Option key={valType.preconId}>
                    id:{valType.preconId} ope: {valType.preconLogicaloperand}
                  </Option>
                ))}
            </Select>
            {/*<div>
          ...
          <Col xs={12} md={12}>
            <LabelInput labelText={"Threshold"} nameInput={"thresholdId"} />
            <Select
              className="ant-select-selection"
              value={formik.values.thresholdId}
              onChange={thesholdHandleChange}
            >
              {thresholdData &&
                thresholdData.map((thres) => (
...
export default RemoteCondition;